        ...
        for entry in data:
            ...
            if metadata_str:
                try:
                    ...
                    # Handle cases where metadata_ is not valid JSON
                    pass
    
    with open(output_file, 'w') as f:
        json.dump(kanana_phones, f, indent=4)

if __name__ == "__main__":
    input_json_file = "phone_data.json"
    output_json_file = "majaneng_phone.json"
    extract_kanana_phones(input_json_file, output_json_file)
    print(f"Extracted majaneng phones saved to {output_json_file}")
